...
from agents.matching_agent import store_job_embedding, match_candidate_to_job,match_candidate_to_specific_job
...
def add_candidate(
    ...
    return {"message": "Candidate added", "id": candidate.id}


@router.post("/jd/store/{job_id}")
def store_jd(job_id: str, jd: JDInput):
    store_job_embedding(job_id, jd.jd_text)
    return {"message": "JD stored"}
...
def match_specific_job(candidate_id: int, job_id: str, session: Session = Depends(get_session)):
    candidate = session.exec(
        select(Candidate).where(Candidate.id == candidate_id)
    ).first()
    jd = session.exec(
    ...
    result = match_candidate_to_specific_job(candidate.resume_text, jd.description,job_id)
    return result